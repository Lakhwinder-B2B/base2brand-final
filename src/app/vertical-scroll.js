...
const MyComponent = () => {
  ...
  useEffect(() => {
    const initializeSlider = () => {
      ...
      if (!sliderElement || !memoriesSection || !headerSection) {
      ...
      if (slider.length > 0) {
        ...
        const handleWheel = (e) => {
          ...
          if (clientHeight > memoriesSectionY && memoriesSectionY > 0) {
            ...
            memoriesSection.style.top = `${headerHeight}px`;
          } else if (clientHeight < memoriesSectionY) {
            memoriesSection.classList.remove("stickyElement");
            memoriesSection.style.top = `unset`;
          }
          ...
          if (memoriesSectionY === headerHeight) {
            ...
            if (firstSection && lastSection) {
              ...
              console.warn("Sections for slider not found.");
            }
          } else {
            $("body").css("overflow", "unset");
          }
        ...
        $('body').on('wheel', handleWheel);

        // Cleanup function to destroy Slick instance and remove event listener if component unmounts
        return () => {
          if (slider.length > 0 && slider.hasClass('slick-initialized')) {
            slider.slick('unslick');
          }
          $('body').off('wheel', handleWheel);
        };
      } else {
        console.warn("Slider element not found or not initialized.");
      }
    };
